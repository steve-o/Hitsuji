...
#include "important_file_writer.hh"
...
bool ImportantFileWriter::WriteFileAtomically(const FilePath& path,
  ...
  if (!ClosePlatformFile(tmp_file)) {
    ...
    chromium::DeleteFile(tmp_file_path, false);
    return false;
  }
  ...
  if (!chromium::ReplaceFile(tmp_file_path, path, NULL)) {
...
ImportantFileWriter::ImportantFileWriter(
    const FilePath& path, chromium::SequencedTaskRunner* task_runner)
        : path_(path),

...
static uint32_t number_of_histograms_ = 0;
// Collect the number of vectors saved because of caching ranges.
static uint32_t number_of_vectors_saved_ = 0;
...
Histogram::Inconsistencies Histogram::FindCorruption(
  ...
  if (delta64 != 0) {
    ...
    if (delta > 0) {
      HISTOGRAM_COUNTS("Histogram.InconsistentCountHigh", delta);
      if (delta > kCommonRaceBasedCountMismatch)
        inconsistencies |= COUNT_HIGH_ERROR;
    } else {
      DCHECK_GT(0, delta);
      HISTOGRAM_COUNTS("Histogram.InconsistentCountLow", -delta);
      if (-delta > kCommonRaceBasedCountMismatch)
...
bool Histogram::HasConstructorTimeDeltaArguments(boost::chrono::milliseconds minimum,
                                                 boost::chrono::milliseconds maximum,
                                                 size_t bucket_count) {
  return ((minimum.count() == declared_min_) &&
          (maximum.count() == declared_max_) &&
          (bucket_count == bucket_count_));
}
...
Histogram::Histogram(const std::string& name, boost::chrono::milliseconds minimum,
                     boost::chrono::milliseconds maximum, size_t bucket_count)
  : histogram_name_(name),
    declared_min_(static_cast<int> (minimum.count())),
    declared_max_(static_cast<int> (maximum.count())),
    bucket_count_(bucket_count),
    flags_(kNoFlags),
    cached_ranges_(new CachedRanges(bucket_count + 1, 0)),
    range_checksum_(0),
...
Histogram::~Histogram() {
  if (DLOG_IS_ON(INFO) && StatisticsRecorder::dump_on_exit()) {
    std::string output;
    WriteAscii(true, "\n", &output);
    DLOG(INFO) << output;
  }
...
uint32_t Histogram::CalculateRangeChecksum() const {
  ...
  // Seed checksum.
  uint32_t checksum = static_cast<uint32_t>(cached_ranges_->size());
  for (size_t index = 0; index < bucket_count(); ++index)
...
StatisticsRecorder::StatisticsRecorder() {
  ...
  if (lock_ == NULL) {
    ...
    lock_ = new chromium::Lock;
  }
  chromium::AutoLock auto_lock(*lock_);
...
void StatisticsRecorder::RegisterOrDeleteDuplicateRanges(Histogram* histogram) {
  ...
  uint32_t checksum = histogram->range_checksum();
...
// static
chromium::Lock* StatisticsRecorder::lock_ = NULL;
// static

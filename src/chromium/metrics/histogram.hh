...
#ifndef CHROMIUM_METRICS_HISTOGRAM_H_
#define CHROMIUM_METRICS_HISTOGRAM_H_
#pragma once
...
/*
  do {
    ...
    static chromium::subtle::AtomicWord atomic_histogram_pointer = 0;

    // Acquire_Load() ensures that we acquire visibility to the pointed-to data
    // in the histogrom.
    chromium::Histogram* histogram_pointer(reinterpret_cast<chromium::Histogram*>(
        chromium::subtle::Acquire_Load(&atomic_histogram_pointer)));
    ...
    if (!histogram_pointer) {
      ...
      histogram_pointer = chromium::Histogram::FactoryGet(
          name, min, max, bucket_count, chromium::Histogram::kNoFlags);
      ...
      chromium::subtle::Release_Store(&atomic_histogram_pointer,
          reinterpret_cast<chromium::subtle::AtomicWord>(histogram_pointer));
    }
...
#define HISTOGRAM_TIMES(name, sample) HISTOGRAM_CUSTOM_TIMES( \
    name, boost::chrono::duration_cast<boost::chrono::milliseconds>(sample), boost::chrono::milliseconds(1), \
    boost::chrono::seconds(10), 50)
...
#define HISTOGRAM_CUSTOM_COUNTS(name, sample, min, max, bucket_count) \
    STATIC_HISTOGRAM_POINTER_BLOCK(name, Add(sample), \
        chromium::Histogram::FactoryGet(name, min, max, bucket_count, \
                                    chromium::Histogram::kNoFlags))
...
#define HISTOGRAM_CUSTOM_TIMES(name, sample, min, max, bucket_count) \
    STATIC_HISTOGRAM_POINTER_BLOCK(name, AddTime(sample), \
        chromium::Histogram::FactoryTimeGet(name, min, max, bucket_count, \
                                        chromium::Histogram::kNoFlags))
...
#define HISTOGRAM_ENUMERATION(name, sample, boundary_value) \
    STATIC_HISTOGRAM_POINTER_BLOCK(name, Add(sample), \
        chromium::LinearHistogram::FactoryGet(name, 1, boundary_value, \
            boundary_value + 1, chromium::Histogram::kNoFlags))
...
// function |chromium::CustomHistogram::ArrayToCustomRanges(samples, num_samples)|
...
#define HISTOGRAM_CUSTOM_ENUMERATION(name, sample, custom_ranges) \
    STATIC_HISTOGRAM_POINTER_BLOCK(name, Add(sample), \
        chromium::CustomHistogram::FactoryGet(name, custom_ranges, \
                                          chromium::Histogram::kNoFlags))
...
class Histogram : boost::noncopyable {
 public:
  ...
  void AddTime(boost::chrono::milliseconds time) {
    Add(static_cast<int>(time.count()));
  }
  ...
  virtual bool HasConstructorTimeDeltaArguments(boost::chrono::milliseconds minimum,
                                                boost::chrono::milliseconds maximum,
                                                size_t bucket_count);
 ...
 protected:
  ...
  Histogram(const std::string& name, boost::chrono::milliseconds minimum,
            boost::chrono::milliseconds maximum, size_t bucket_count);
...
class LinearHistogram : public Histogram {
 ...
 protected:
  ...
  LinearHistogram(const std::string& name, boost::chrono::milliseconds minimum,
                  boost::chrono::milliseconds maximum, size_t bucket_count);
...
class StatisticsRecorder : boost::noncopyable {
 public:
 ...
 private:
  ...
  // |cached_ranges_|.
  typedef std::map<uint32_t, std::list<CachedRanges*>*> RangesMap;
...
class CachedRanges : boost::noncopyable {
 public:
  ...
  uint32_t range_checksum(uint32_t checksum) const { return range_checksum_; }
  void SetRangeChecksum(uint32_t checksum) { range_checksum_ = checksum; }
...
#endif  // CHROMIUM_METRICS_HISTOGRAM_H_

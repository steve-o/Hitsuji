...
StatsCounterTimer::StatsCounterTimer(const std::string& name) : has_start_time_ (false), has_stop_time_ (false)
{
...
void StatsCounterTimer::Stop() {
  if (!Enabled() || !Running())
    return;
  stop_time_ = boost::chrono::high_resolution_clock::now();
  has_stop_time_ = true;
  Record();
}
...
bool StatsCounterTimer::Running() {
  return Enabled() && has_start_time_ && !has_stop_time_;
}

// Accept a TimeDelta to increment.
void StatsCounterTimer::AddTime(boost::chrono::milliseconds time) {
  Add(static_cast<int>(time.count()));
}

void StatsCounterTimer::Record() {
  AddTime(boost::chrono::duration_cast<boost::chrono::milliseconds>(stop_time_ - start_time_));
}

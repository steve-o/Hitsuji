...
class StatsTable::Private {
 public:
  ...
  static Private* New(const std::string& name, int size, int max_threads, int max_counters);
  ...
  int32_t* thread_tid(int slot_id) const {
    return &(thread_tid_table_[slot_id-1]);
  }
  int32_t* thread_pid(int slot_id) const {
 ...
 private:
  ...
  void InitializeTable(void* memory, int size, int max_counters, int max_threads);
  ...
  int32_t* thread_tid_table_;
  int32_t* thread_pid_table_;
...
StatsTable::Private* StatsTable::Private::New(const std::string& name,
                                              ...
                                              int max_threads,
                                              int max_counters) {
  std::unique_ptr<Private> priv(new Private());
...
void StatsTable::Private::ComputeMappedPointers(void* memory) {
  ...
  thread_tid_table_ = reinterpret_cast<int32_t*>(data + offset);
  offset += sizeof(int32_t) * max_threads();
  offset += AlignOffset(offset);

  thread_pid_table_ = reinterpret_cast<int32_t*>(data + offset);
  offset += sizeof(int32_t) * max_threads();
  offset += AlignOffset(offset);
...
StatsTable::StatsTable(const std::string& name, int max_threads,
  ...
  int table_size =
    ...
    AlignedSize(max_threads * sizeof(int32_t)) +
    AlignedSize(max_threads * sizeof(int32_t)) +
  ...
  if (!impl_)
    DLOG(ERROR) << "StatsTable did not initialize";
}
...
int StatsTable::GetRowValue(int index, int32_t pid) const {
  if (!impl_)
    return 0;
...
int StatsTable::GetCounterValue(const std::string& name, int32_t pid) {
  if (!impl_)
    return 0;
